...
- `/flows` **Happy paths**: `buy-chain` / `buy-chain-eoa` / `tip-chain` / `tip-chain-eoa` — land → intent → auth → top up → complete. Tip chain morphs top-up success into **Buy to tip** (USDm/ETH/WETH + Keep portion) before the tip settles. Funds confetti suppressed mid-chain; single celebration at tip/buy end. Guided with a fake cursor + button pulse (GSAP). Floating **Friction** counter tallies guided actions, then hides so you can take over.
...
### Docs
- README [#tipping](https://github.com/frilo-eth/kby-feed/blob/main/README.md#tipping): bought position, Buy to tip, orange until the bag is empty. Happy-path tip chain and friction hide. Stakeholder flows + deposit `buyToTip` step.
